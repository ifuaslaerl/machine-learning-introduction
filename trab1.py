...
from src.models_settings import options_classifiers
...
if __name__ == "__main__":
    ...
    for index, obj in enumerate(objs):
        ...
        # colunas = [coluna for coluna in treated_data.columns if coluna != "id_solicitante"]
        ...
        # menor = inf
        # for jndex, element in enumerate(matriz["inadimplente"]):
        #     if element < menor:
        #         menor = element
        #         sla = jndex
        # print(treated_data.columns[sla], menor)
        
        precisao = accuracy_score(test["inadimplente"], answer["inadimplente"])
    ...
    for index, (porcentagem , model) in enumerate(results):
        ...
        precisao_ = accuracy_score(validacao["inadimplente"], answer["inadimplente"])
        #model.view_config()
        tempo = str(timedelta(seconds=time()-start))
    ...
    for index, (porcentagem, model) in enumerate(result_final):
        ...
        answer.to_csv(f"data/trabalho1/results/{index}.csv", index=False)
